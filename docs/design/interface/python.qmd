...
Both `build()` and `view()` functions read in the `datapackage.json`
file through the `read_properties()` function from the `seedcase_sprout`
package, which returns a `PackageProperties` object. This is a Python
class that represents the properties of a datapackage, and is used as
...
-   Provides a clear and structured way to represent the properties of a
    Data Package, with defined attributes and types.
-   Allows for better type checking of the properties and ensures the

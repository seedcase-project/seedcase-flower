...
::: content-hidden
This `Properties` class is used instead of a dictionary for a few reasons:
...
- `uri` for the URI argument. The default is the `datapackage.json` file
  found in the working directory. See the [CLI design](cli.qmd) for
  reasons why we use a URI.
...
def build(
    ...
    # TODO: I believe cylcopts can handle the config loading of CLI options, so
    # we might not need to implement this ourselves.

    # If any config option is provided, use that, not the config file.
    # If there is no config option, look for config file in same dir as
    # `datapackage.json`. Lastly, if none of the above, use the default style.
    config: Config = Config(
        style=style,
        template_dir=template_dir,
        output_dir=output_dir
    )
    # Pseudocode, this might not be necessary.
    if Config is all None:
        # if `template_dir` is given.
    ...
    output_files: list[Path] = write_sections(output, config.output_dir)
...
that are both the same as those in the `build()` function: `uri` and

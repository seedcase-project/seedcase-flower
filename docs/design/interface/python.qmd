...
It takes the same parameters as `build()`: `source` and `style`. Unlike
`build()`, `view()`'s `style` parameter only accepts built-in one-page
styles (the terminal displays a single page) and ignores custom styles
and the configuration file. For details about the parameters, see

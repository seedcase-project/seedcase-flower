...
-   `template_dir` for directory that contains the custom styling Jinja
    template files as well as the `sections.toml` file. Defaults to
    `None` as the default style is a built-in style that uses built-in
    templates.
...
def build(
    ...
    if Config is all None:
        # Within `load_config()` is also loading of the `sections.toml` file,

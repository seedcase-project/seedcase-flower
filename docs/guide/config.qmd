...
If there are no configuration settings given in either the CLI, the
Python interface, or in the configuration file, Flower will use its
defaults.

::: callout-note
The source can only be specified via the CLI or the Python interface,
not from the configuration file. Similarly, you can only configure the

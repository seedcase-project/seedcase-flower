...
There are two ways to configure Flower: As options on the command line interface (CLI), when using the Python interface directly,
or in the configuration file. If you use the CLI (or the Python interface) to
configure Flower's output, Flower **will not** use the settings in the configuration file. If there are no configuration settings given in either the
CLI, the Python interface, or in the configuration file, Flower will use it's defaults.

::: callout-note
The URI is only given on the CLI or from the Python interface and 
not from the configuration file. Similarly, you can only configure
the `view` command from the CLI or the Python interface, not the configuration file.
:::
...
can find the settings. Open up `pyproject.toml` and add a new table header to the end
as `[tool.seedcase-flower]`. Then put the same config settings as you used above
in `.flower.toml` below this table header.

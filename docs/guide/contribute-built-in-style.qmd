...
`ViewStyle`) in `src/seedcase_flower/styles.py`. All styles are
defined in the `Style` enum, while styles suitable for terminal
output with `view` are also added to `ViewStyle`. For example, if the

...
::: callout-note
The `view` command can display built-in styles with one or more sections.
Multi-section styles are rendered as one paged terminal document, with
each section labelled by its `output-path`.
:::
...
enum. For example, if `your_new_style` is your new style, you would include
it in `Style` like so:
